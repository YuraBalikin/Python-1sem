...
message = ['Твоя очередь', 'Ну бери уже', 'Бери больше', 'Быстрее']
...
def player_vs_player():
    ...
    print(f'\nА теперь {player_1} и  {player_2} начнется игра !\n')
    ...
    if x == 1:
        ...
        lucky = player_2
        loser = player_1
    print(f'Поздравляю {lucky} рандом выбрал тебя, ты ходишь первым !')
    ...
    while candies_total > 0:
        if count == 0:
            ...
            if step > candies_total or step > max_take:
                step = int(input(
                    f'\nНе смотрел правила? Можно взять только {max_take} конфет {lucky}, бери заново: '))
            candies_total = candies_total - step
        if candies_total > 0:
            print(f'\nОстается еще {candies_total}')
            count = 1
        else:
            print('Все? И правда. Конфет больше нет!')

        if count == 1:
            step = int(input(f'\n{choice(message)}, {loser} :'))
            if step > candies_total or step > max_take:
                step = int(input(
                    f'\nНечитал правила? Можно взять только {max_take} конфет {lucky}, бери заново: '))
            candies_total = candies_total - step
        if candies_total > 0:
    ...
    while candies_total > 0:
        ...
        if players[lucky % 2] == 'Терминатор':
            ...
            while step > max_take or step > candies_total:
                step = int(input(f'\nЗа один ход можно взять {max_take} конфет, бери заново: '))
        candies_total = candies_total - step
